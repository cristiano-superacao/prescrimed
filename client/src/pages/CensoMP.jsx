...
export default function CensoMP() {
  ...
  const loadData = async () => {
    try {
      ...
      const prescricoes = Array.isArray(prescricoesData)
        ? prescricoesData
            ...
            <table className="w-full">
              <thead className="bg-slate-50 border-b border-slate-100">
                <tr>
                  ...
                  <th className="px-6 py-4 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Prescrições Ativas</th>
                    ...
                    <td className="px-6 py-4">
                      {item.hasPrescription ? (
                        <div className="flex flex-col gap-1">
                          {item.prescriptions.map((prescricao) => {
                            const key = prescricao.id || prescricao._id;
                            const meds = prescricao.medicamentos || [];
                            const principal = meds[0]?.nome || 'Prescrição sem medicamentos cadastrados';
                            const extra = meds.length > 1 ? ` +${meds.length - 1}` : '';

                            return (
                              <span
                                key={key}
                                className="text-xs text-slate-500 bg-slate-100 px-2 py-1 rounded inline-block w-fit"
                              >
                                {principal}{extra}
                              </span>
                            );
